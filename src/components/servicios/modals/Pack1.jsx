...
export default function Pack1({ data }) {
  const nivelesKeys = ["basico", "intermedio", "premium"];
    ...
    <div className="text-white flex flex-col items-center gap-6 relative">
      ...
      <div className="flex justify-center gap-3 flex-wrap">
        {nivelesKeys.map((key, index) => (
          ...
          >
            {data.niveles[key].nombre.split("–")[0].trim()}
          </button>
        ))}
      </div>

      {/* Botón comparativa centrado con ícono (desktop/tablet) */}
      ...
      {/* Modal secundario: tabla comparativa */}
