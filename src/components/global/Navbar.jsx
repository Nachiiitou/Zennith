...
const Navbar = ({ activeSection, toggleLanguage, lang }) => {
  ...
  return (
    <nav className="relative flex items-center px-6 py-5 border-b border-[#1de9b6] bg-[#02070f]/90 backdrop-blur-sm z-10">
      {/* Logo */}
      <div
      ...
      <ul className="hidden lg:flex absolute left-1/2 transform -translate-x-1/2 gap-10 text-base">
        {["hero", "servicios", "nosotros", "contacto"].map((item, idx) => {
          ...
          return (
            ...
            >
              {["servicios", "nosotros", "contacto"].includes(item) ? (
                <Link
                  to={`/${lang}/${item}`}
      ...
      <div className="flex gap-3 items-center flex-none ml-auto">
        <button
          ...
          className="hidden lg:block text-sm border border-[#1de9b6] px-4 py-1 rounded-full text-[#1de9b6] hover:bg-[#1de9b6] hover:text-black transition cursor-pointer"
        >
          {lang === "es" ? "ES" : "EN"}
        </button>
